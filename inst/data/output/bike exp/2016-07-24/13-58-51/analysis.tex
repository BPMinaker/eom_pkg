...
\begin{center}
\begin{tikzpicture}
\begin{axis}[height=3in,width=5in,xmin=0,zmin=0,restrict y to domain=-100:100,restrict expr to domain={abs(y)}{0.001:100},xlabel={Speed [m/s]},ylabel={Eigenvalue [1/s]},tick style={thin,black},extra y ticks={0},extra y tick style={grid=major},major y grid style={dotted,black},enlarge x limits=false,legend style={at={(1.0,1.03)},anchor=south east},legend columns=-1]
\addplot3[only marks,mark=*,mark options={black,scale=0.6,line width=1pt}] table[x=speed,y=real,z=imag]{eigen.out};
...
\end{tikzpicture}
\end{footnotesize}
\caption{Frequency response: yaw rate/steer torque}
\label{bode_plot_1}
\end{center}
\end{figure}
...
\end{tikzpicture}
\end{footnotesize}
\caption{Frequency response: roll angle/steer torque}
\label{bode_plot_2}
\end{center}
\end{figure}
