function the_system=input_cook_2(n,varargin)
the_system.item={};

% Copyright (C) 2008, Bruce Minaker
%% This file is intended for use with Octave.
%% input_cook_2.m is free software; you can redistribute it and/or modify it
%% under the terms of the GNU General Public License as published by
%% the Free Software Foundation; either version 2, or (at your option)
%% any later version.
%%
%% input_cook_2.m is distributed in the hope that it will be useful, but
...
for i=0:xdivs
	for j=0:ydivs
		...
		body.location=[x(i+1,j+1);y(i+1,j+1);0];
		body.name=['body-' num2str(i) num2str(j)];
		the_system.item{end+1}=body;

		point.location=[x(i+1,j+1);y(i+1,j+1);0];
...
for i=1:xdivs
	for j=1:ydivs
		pt1=[x(i,j);y(i,j);0];
		pt2=[x(i+1,j);y(i+1,j);0];
		pt3=[x(i,j+1);y(i,j+1);0];
		pt4=[x(i+1,j+1);y(i+1,j+1);0];
...
for j=0:ydivs
		...
		point.location=[x(1,j+1);y(1,j+1);0];
