...
len=norm(d);
if(len>0)
	d=d/len;  %% Find the unit vector along the two points
	...
	if(an)
		ax=ax/an;  %% Find that unit vector if possible
	else
	...
	if(aa(4)<0)  %% To get positive rotations, we choose the negative axis
		aa=-aa;
	end
else
	aa=[0;0;1;0];
