function [v_mtx,mv_mtx]=centngyro(in) %% function 'mass' returns 'mtx' as a function of 'bodys' [from eom.m]
...
for i=1:n  %% For each body coming in
	...
	mv_mtx(6*i+(-2:0),6*i+(-2:0))=-skew(in(i).inertia*in(i).angular_velocity);
